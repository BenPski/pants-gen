use std::{error::Error, process::exit, usize};

use clap::Parser;
use pants_gen::{interval::Interval, password::Password};
...
#[command(
    ...
    after_long_help = "General formatting follows the style of CHARSET|INTERVAL\
...
struct CliArgs {
    ...
    #[arg(short, long)]
    length: Option<usize>,
    /// constraints on uppercase characters
    #[arg(short, long)]
    upper: Option<Interval>,
    /// constraints on lowercase characters
    #[arg(short = 'd', long)]
    lower: Option<Interval>,
    /// constraints on number characters
    #[arg(short, long)]
    number: Option<Interval>,
    /// constraints on symbols characters
    #[arg(short, long)]
    symbol: Option<Interval>,
    /// constraints on custom characters
...
fn main() {
    ...
    println!("{:?}", password_spec);
    let password = password_spec.generate();
